...
def _jwk_from_pubkey(pub) -> dict:
    ...
    if isinstance(pub, ec.EllipticCurvePublicKey):
        ...
        name = pub.curve.name
        return {"kty": "EC", "crv": crv_map.get(name, name), "x": _b64u(x), "y": _b64u(y)}
    if isinstance(pub, ed25519.Ed25519PublicKey):
        return {"kty": "OKP", "crv": "Ed25519", "x": _b64u(pub.public_bytes(Encoding.Raw, serialization.PublicFormat.Raw))}
    if isinstance(pub, x25519.X25519PublicKey):
        return {"kty": "OKP", "crv": "X25519", "x": _b64u(pub.public_bytes(Encoding.Raw, serialization.PublicFormat.Raw))}
...
def _load_x509_from_data(data: bytes, fmt: str, password: bytes | None) -> x509.Certificate:
    fmt = fmt.upper()
    if fmt == "PEM":
        try:
...
def jwk_from_certificate(file_bytes: bytes, fmt: str, password: str | None = None) -> tuple[dict, str]:
    cert = _load_x509_from_data(file_bytes, fmt, password.encode() if password else None)
    pub = cert.public_key()
