...
class CertificatesController:
    ...
    def upload(self, request,
        ...
        if fmt not in allowed:
            raise HttpError(400, "Unsupported certificate format. Use: PEM, DER, PKCS7, PKCS12, CRT, AUTO.")
        data = file.read()
        ...
        cert_row = Certificate.objects.create(
            owner=request.user, organization=org, file=file, format=eff_fmt,
            extracted_jwk=jwk, fingerprint=fingerprint
        )
        return ok(request,
                  did_state={"state": "action"},
                  did_doc_meta={"certificate_id": str(cert_row.id), "public_jwk": cert_row.extracted_jwk, "fingerprint": fingerprint},
