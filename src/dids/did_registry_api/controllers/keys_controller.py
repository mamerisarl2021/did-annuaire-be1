...
class KeysController:
    ...
    def rotate_key(self, request, did: str, body: dict = Body(...)):
        ...
        if not ref_doc:
            return err(request, 404, "No existing document to infer key_id from",
                       path=f"/api/registry/dids/{did}/keys/rotate")

        vms = (ref_doc.document.get("verificationMethod") or [])
        if not isinstance(vms, list) or len(vms) != 1:
            return err(request, 400, "SINGLE_VM_REQUIRED: cannot infer key_id when verificationMethod!=1",
                       path=f"/api/registry/dids/{did}/keys/rotate")

        vm_id = vms[0].get("id") or ""
        key_id = vm_id.split("#", 1)[1] if "#" in vm_id else vm_id
        if not key_id:
            return err(request, 400, "CANNOT_INFER_KEY_ID from verificationMethod.id",
                       path=f"/api/registry/dids/{did}/keys/rotate")

        # Bump key material version for inferred key_id
        latest_ver = (UploadedPublicKey.objects.filter(did=did_obj, key_id=key_id)
        ...
        try:
            ...
            return err(request, 400, str(ve), path=f"/api/registry/dids/{did}/keys/rotate")
        ...
        did_doc = DIDDocument.objects.create(
            did=did_obj, version=next_version, document=document, environment="DRAFT", is_active=False
        )
        ...
        return ok(
            request,
            did_state={"state": "update", "did": did_obj.did, "didDocument": document,
