...
def activate_prod(did_obj, new_doc_model: DIDDocument) -> None:
    did_obj.documents.filter(environment="PROD", is_active=True).exclude(pk=new_doc_model.pk).update(is_active=False)
...
def publish_to_prod(doc_model: DIDDocument) -> str:
    ...
    org = getattr(did.organization, "slug", None) or getattr(did.organization, "namespace", None) or str(
        did.organization_id)
    user = getattr(did.owner, "slug", None) or getattr(did.owner, "username", None) or str(did.owner_id)
    ...
    doc_model.save(update_fields=["file_sha256", "file_etag", "published_relpath", "environment"])
