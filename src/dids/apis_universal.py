...
from .services import draft_fill_hash, publish_preprod, publish_prod, deactivate_did
...
class UniversalRegistrarController:
    ...
    def create(self, request, method: str, payload: CreateRequest) -> RegistrarState:
        ...
        did_doc = DIDDocument.objects.create(did=did, version=version, document=doc, environment="DRAFT",
                                             is_active=False)
        ...
        return RegistrarState(
            ...
            didDocumentMetadata={"created": (
                did_doc.created_at.isoformat() if hasattr(did_doc, "created_at") and did_doc.created_at else None)},
    ...
    def update(self, request, payload: UpdateRequest) -> RegistrarState:
        ...
        version = (did.documents.aggregate(Max("version")).get("version__max") or 0) + 1
        did_doc = DIDDocument.objects.create(did=did, version=version, document=new_doc, environment="DRAFT",
                                             is_active=False)
        draft_fill_hash(did_doc)
        ...
        return registrar_ok(
                        ...
                        did_doc_meta={"versionId": str(did_doc.version), "environment": "DRAFT", "published": False},
                        status=200
                    )
    ...
    def deactivate(self, request, payload: DeactivateRequest) -> RegistrarState:
        ...
        DIDDocument.objects.create(did=did, version=version, document=deactivated_doc, environment="PROD",
                                   is_active=True)
...
class UniversalResolverController:
    ...
    def resolve(self, request, identifier: str):
        ...
        doc_model = did.documents.filter(environment="PROD", is_active=True).order_by("-version").first()
        ...
        if accept == "application/did-resolution":
            body = {
                ...
                "didDocumentMetadata": {"versionId": (str(doc_model.version) if doc_model else None)},
            }
