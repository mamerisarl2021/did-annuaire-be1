    ...
    def resolve(self, request, identifier: str):
        ...
        if _wants_resolution(accept):
            ...
            try:
                ...
                parser = {"did": identifier, "method": "web", "method_id": f"{host}:{org}:{user}:{doc_type}",
                          "query": request.GET.get("c", "Not available")}
            except Exception:
                parser = {"did": identifier, "method": "Not available", "method_id": "Not available",
                          "query": request.GET.get("c", "Not available")}

            # Services block
            services = doc.get("service")
            services_block = ([{"id": s.get("id"), "type": s.get("type"), "service_endpoint": s.get("serviceEndpoint")}
                               for s in services] if isinstance(services, list) and services else "NOT AVAILABLE")

            # Verification methods block
            vms = [{"id": vm.get("id"), "type": vm.get("type"), "public_key_jwk": vm.get("publicKeyJwk")}
            ...
            return JsonResponse(res, status=200, content_type="application/did-resolution+json")
