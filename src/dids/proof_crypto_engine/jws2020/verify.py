...
def _pubkey_from_jwk(jwk: dict):
    ...
    if kty == "EC":
        ...
        crv = jwk.get("crv")
        curve = {
            "P-256": ec.SECP256R1(), "secp256r1": ec.SECP256R1(),
            "P-384": ec.SECP384R1(), "secp384r1": ec.SECP384R1(),
            "P-521": ec.SECP521R1(), "secp521r1": ec.SECP521R1(),
        }.get(crv)
        if not curve:
...
def _encode_ecdsa_der(r: int, s: int) -> bytes:
    ...
    rb = int_to_bytes(r); sb = int_to_bytes(s)
    seq = b"\x02" + bytes([len(rb)]) + rb + b"\x02" + bytes([len(sb)]) + sb
    return b"\x30" + bytes([len(seq)]) + seq

def verify_detached_jws(protected_b64: str, signature_b64: str, payload_str: str, jwk: dict) -> bool:
    try:
