...
def _first_cert_from_pkcs7(data: bytes, is_pem: bool) -> Optional[x509.Certificate]:
    try:
        certs: List[x509.Certificate] = pkcs7.load_pem_pkcs7_certificates(data) if is_pem else pkcs7.load_der_pkcs7_certificates(data)
        return certs[0] if certs else None
    except Exception:
        return None

def load_x509(file_bytes: bytes, fmt: str, password: Optional[str] = None) -> x509.Certificate:
    f = (fmt or "").upper()
    if f == "PEM":
        try:
    ...
    if f == "PKCS7":
        ...
        return c
    if f == "PKCS12":
        key, cert, _extra = pkcs12.load_key_and_certificates(file_bytes, password.encode() if password else None)
        if cert is None:
