...
from .services import draft_fill_hash, build_host, derive_org_slug, derive_user_slug, build_relpath, \
    jcs_canonical_bytes, sha256_hex
...
class DIDCreationController:
    ...
    def create_did(self, request, payload: CreateDIDRequest):
        ...
        owner = get_object_or_404(User, pk=payload.owner_id) if payload.owner_id else request.user
        ...
        if payload.keys:
            ...
            for k in payload.keys:
                ...
                cert = get_object_or_404(Certificate, pk=k.certificate_id, organization=org)
                key_specs.append({"certificate": cert, "key_id": k.key_id, "purposes": k.purposes})
        else:
            # legacy single-key
            if not (payload.certificate_id and payload.key_id):
                raise HttpError(400, "Provide either keys[] or (certificate_id & key_id)")
            cert = get_object_or_404(Certificate, pk=payload.certificate_id, organization=org)
            key_specs.append({"certificate": cert, "key_id": payload.key_id, "purposes": payload.purposes})

        # Build DID + DID Document (with strict purpose validation in builder)
        try:
            if payload.keys:
                did_str, document = build_did_document_with_keys(
                    organization=org, owner=owner, document_type=payload.document_type,
                    keys=[{"jwk": ks["certificate"].extracted_jwk, "key_id": ks["key_id"], "purposes": ks["purposes"]} for ks in key_specs],
                    services=payload.services,
                )
            else:
                did_str, document = build_did_and_document(
                    organization=org, owner=owner, document_type=payload.document_type,
                    jwk=key_specs[0]["certificate"].extracted_jwk, key_id=key_specs[0]["key_id"],
                    purposes=key_specs[0]["purposes"], services=payload.services,
                )
        ...
        did = DID.objects.create(did=did_str, organization=org, owner=owner, document_type=payload.document_type)

        upk_ids = _persist_keys(did, key_specs)

        did_doc = DIDDocument.objects.create(did=did, version=1, document=document, environment="DRAFT", is_active=False)
    ...
    def preview_did(
        ...
        try:
            if certificate_id and key_id:
                ...
                did_str, document = build_did_and_document(org, owner, document_type, jwk=cert.extracted_jwk,
                                                           key_id=key_id, purposes=purposes)
            else:
