...
from src.api_keys.presenters import api_key_to_list_dto, api_key_created_dto
...
class APIKeyController(ControllerBase):
    ...
    def create_api_key(self, name: str, permissions: list[str], expires_at: str = None):
        ...
        if user.role not in [UserRole.ORG_ADMIN, UserRole.SUPERUSER]:
            return self.create_response(message='Only ORG_ADMIN can create API keys', extra={}, status_code=403)

        api_key, plain_key = services.api_key_create(organization=user.organization,
                                                     created_by=user,
                                                     name=name,
    ...
    def revoke_api_key(self, api_key_id: int):
        ...
        if user.role not in [UserRole.ORG_ADMIN, UserRole.SUPERUSER]:
            return self.create_response(message='Only ORG_ADMIN can revoke API keys', extra={}, status_code=403)
        ...
        return self.create_response(message='API key revoked successfully', extra={}, status_code=200)
