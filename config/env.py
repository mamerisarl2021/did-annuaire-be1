...
OPENBAO_KV_PATH = env(
    "OPENBAO_KV_PATH", default="django"
)  # e.g., "django", or "django/prod" if you split per env
...
def bao_read_kv(path=None):
    ...
    resp = c.secrets.kv.v2.read_secret_version(
        mount_point=OPENBAO_KV_MOUNT, path=target_path
    )
...
def env_get(name: str, default=None, *, kv_path=None, prefer_env: bool = True):
    ...
    try:
        ...
        log.warning(
            "env_get: OpenBao fallback failed for %s: %s (using default)", name, e
        )
    return default
